...
function My_account_vendor() {
    ...
    const [avatarUrl, setAvatarUrl] = useState(
        "https://api.dicebear.com/9.x/initials/svg?seed=Shipper" // default avatar
    );
    ...
    // Top heading message (changes when saving / logging out)
    const [announcement, set_announcement] = useState(["My account", "text-white"])
    // Ref to track avatar preview URL (so we can clean up later)
                ...
                <section className="rounded-2xl border border-white bg-black/60">
                    ...
                    <dl className="grid grid-cols-1 gap-6 p-6 sm:grid-cols-2">
                        ...
                        <div className="rounded-xl border border-white bg-black/60 p-4 sm:col-span-2">
                            <dt className="text-xs uppercase tracking-wide text-white/70">Business name</dt>
                            <dd className="mt-1 text-base">{business_name}</dd>
                        </div>

                        <div className="rounded-xl border border-white bg-black/60 p-4 sm:col-span-2">
                            <dt className="text-xs uppercase tracking-wide text-white/70">Business address</dt>
                            <dd className="mt-1 text-base">{business_address}</dd>
                        </div>
                    </dl>
                </section>
                {/* Log out button */}
...
export default My_account_vendor;
