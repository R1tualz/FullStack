...
router.get("/get_user", async (request, response) => {
    try {
        ...
        if (request.session.account_type === "Customer") {
            response.status(200).json({ username: user_data.username, avatar: avatarUrl, name: user_data.name, address: user_data.address, account_type: user_data.account_type, status: true })
        }
        else if (request.session.account_type === "Shipper") {
            response.status(200).json({ username: user_data.username, avatar: avatarUrl, distribution_hub: user_data.distribution_hub, account_type: user_data.account_type, status: true })
        }
        else { // Vendor
            response.status(200).json({ username: user_data.username, avatar: avatarUrl, business_name: user_data.business_name, account_type: user_data.account_type, business_address: user_data.business_address, status: true })
        }
    }
    catch (err) {
